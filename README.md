...
or the interview bank and then pushing is enough to ship. You do not need to rebuild locally.
...
Add it under **Settings → Secrets and variables → Actions**. (The site ID is not secret. It already sits
...
- The genetic-modification framing holds throughout: **work only in a legal, registered space.**
...
openable, and completable from the first visit, whatever you finished before:
...
- Dependencies stay as **guidance**, not gates. They still draw the graph and populate
...
- Node states are only ever **done**, **next up** (a suggested next step whose prerequisites you
  you happen to finish), or **open**. "Open" is a soft de-emphasis, never a barrier. You can
...
23 videos and 147 resources all sit inline, so it needs no build step, no database and no
internet. Fonts and videos load online when they can, and the app itself works offline.
...
- **List view** — a searchable, tappable index grouped by tier. It is the default on phones (≤720px).
- **Search** filters both views. **Progress** (mark-complete, then the cascade) persists in `localStorage`.
...
- The theme **follows your OS** until you click the toggle. An explicit
...
re-resolves on a theme change. Do not replace those with literal colors.
...
`foundational` / `core` / `advanced`, anchored to the tree node they test). The questions read
...
To extend the bank, add objects to `interview_questions.json`. `nodeId` must match a skill-node id, and
...
- An **optional equipment side-tree** — `micropipette`, `centrifuge`, `thermocycler`,
  ...
  nothing on the main spine depends on them, so you can browse them freely in parallel without
...
**Videos are wired in.** The `video.url` field carries a value for **23** techniques, each a verified,
...
The included generator writes every output file (pure Python 3, no dependencies):
