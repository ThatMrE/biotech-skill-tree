#!/usr/bin/env python3
"""
!!! LEGACY / ARCHIVAL - DO NOT RUN TO REGENERATE THE CURRENT TREE !!!

This is the ORIGINAL generator that produced the first 43-node core curriculum.
The tree has since grown to 59 nodes (5 HTGAA modules + an 11-node emerging-skills
tier), and those additions are NOT in this script. Running it would emit only the
old 43-node subset.

The JSON files under src/data/ are now the SINGLE SOURCE OF TRUTH:
    src/data/trees/synthetic-biology.json   - the node graph
    src/data/skill_nodes/*.json             - per-node detail
    src/data/interview_questions.json       - interview bank
    src/data/htgaa_companion.json           - HTGAA week mapping

To change content, edit those files and run `python build_viewer.py`.
(This script writes to a hardcoded /home/claude/skilltree_out path, so it cannot
overwrite the repo in place - it is kept only for provenance.)
