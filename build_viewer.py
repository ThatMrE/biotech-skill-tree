...
(function(){
  ...
  function openPop(id){
    ...
    // reflect the open node in the URL so it can be shared / deep-linked (/#node-id).
    // replaceState does NOT fire hashchange, so this never re-triggers openFromHash.
  ...
  function closePop(){scrim.classList.remove('show');
    // drop the #node-id from the URL when the panel closes (no hashchange fired)
  ...
  // ---- deep-linking: open a node straight from the URL hash (e.g. /#crispr) ----
  function nodeIdFromHash(){
    var h=(location.hash||'').replace(/^#/,'');
    try{h=decodeURIComponent(h);}catch(e){}
    return (h&&byId[h])?h:null;   // only honour a hash that names a real node
  }
  function openFromHash(){
    var id=nodeIdFromHash();
    if(id)studyNode(id);          // switch to a sensible view and open the node's detail
  }
  // react to the hash changing after load (shared link opened in-tab, back/forward, manual edit)
  ...
  // if the page was opened with /#node-id, jump straight to that node
  openFromHash();
})();
</script>
</body>
